...
#
# In order to reach the princess as quickly as possible, the knight move only rightward or downward in each step.
#
#
#
...
class Solution:
    def calculateMinimumHP(self, dungeon):
        ...
        # health[i][j]: minimum health at dungeon[i][j] so the knight can survive at the bottom right
        # health[i][j] = min(health[i+1][j], health[i][j+1]) - dungeon[i][j]
        health = [[math.inf] * (n + 1) for _ in range(m + 1)]
        # use dummy row and column for calculation
        health[m-1][n], health[m][n-1] = 1, 1

        for i in range(m - 1, -1, -1):
            for j in range(n - 1, -1, -1):
                # notice that if need <= 0, health[i][j] = 1
                need = min(health[i+1][j], health[i][j+1]) - dungeon[i][j]
                health[i][j] = 1 if need <= 0 else need
        return health[0][0]
